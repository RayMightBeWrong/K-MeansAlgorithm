...
void attributeInitialClusters(int N, int K, int THREADS, float *px, float *py, 
	...
	#pragma omp parallel for num_threads(THREADS) shared(point_cluster, px, py, cx, cy)
	for(int i = 0; i < N; i++)
...
int attributeClusters(int N, int K, int THREADS, float *px, float *py, 
	...
	#pragma omp parallel for num_threads(THREADS) private(cluster) shared(point_cluster, changed)
	for(int i = 0; i < N; i++){
...
void rearrangeCluster(int N, int K, int THREADS, float *px, float *py, 
	...
	#pragma omp parallel num_threads(THREADS) firstprivate(local_size, local_x, local_y) shared(size, x, y, point_cluster, px, py)
	{
